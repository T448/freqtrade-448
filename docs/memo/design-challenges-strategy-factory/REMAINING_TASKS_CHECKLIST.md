...
- [x] MLLabelGeneratorクラスを実装 (two_tier_strategy.pyのset_freqai_targets()として実装済み)
- [x] 価格ベース特徴量生成を実装
- [x] テクニカル指標ベース特徴量生成を実装
- [x] リターン計算を実装
- [x] ラベル生成を実装
- [x] テストで検証
...
- [x] 実装を `.shift(-n)` に変更 (既に実装済み atr_breakout.py:113-114, 156-157)
- [x] テストで検証
...
- [x] 実装コードを読んで動作理由を特定 (.shift(-n)使用のため自動的にNaN)
- [x] 元の設計仕様を確認
- [x] すべてNaN or すべて0に統一する実装に修正 (既に統一済み)
- [x] テストで検証
...
- [x] モジュールパスを修正 (strategies.primary.* のままで動作確認済み)
- [x] テストで検証
